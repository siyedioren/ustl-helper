    ...
    Taro.request({
      ...
      fail: function (res) {
        ...
        if (errMsg.includes("timeout")) {
          options.completeLoad = () => Toast.info("请求超时，请稍后重试");
        } else if (errMsg.includes("fail") || errMsg.includes("network") || errMsg.includes("cancel")) {
          options.completeLoad = () => Toast.info("网络错误，请检查网络连接");
        }
        options.fail(res);
      },
